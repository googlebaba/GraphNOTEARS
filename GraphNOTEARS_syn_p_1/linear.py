...
def notears_linear(X, x_temp, A, lambda1, loss_type, max_iter=100, h_tol=1e-8, rho_max=1e+16, w_threshold = 0.5):
    ...
    for _ in range(max_iter):
        ...
        if h <= h_tol or rho >= rho_max:
            break
    W_est = _adj(w_est)
    print("W_est before:",W_est)
    W_est[np.abs(W_est) < w_threshold] = 0
    print("W_est after:", W_est)
    return W_est
...
if __name__ == '__main__':
    ...
    # 生成：x = x_temp + A @ x_temp

    # x_temp
    x_temp = utils.simulate_linear_sem(W_true, n, sem_type)
    ...
    W_est = notears_linear(X, x_temp, A, lambda1=0.1, loss_type='l2')
    print(W_est)
