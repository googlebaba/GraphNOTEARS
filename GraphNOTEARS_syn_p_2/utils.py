...
def simulate_linear_sem_with_P(W, P, P2, lagX, lagX2, n, sem_type, noise_scale=None):
    """Simulate samples from linear SEM with specified type of noise.
    ...
    for j in ordered_vertices:
        ...
        X[:, j] = _simulate_single_equation(X[:, parents], W[parents, j], scale_vec[j]) + _simulate_single_equation(lagX[:, P_parents], P[P_parents, j], scale_vec[j]) + _simulate_single_equation(lagX2[:, P2_parents], P2[P2_parents, j], scale_vec[j])
    return X
