...
class dynotears_MLP(nn.Module):
    def __init__(self, dims, bias=True):
        super(dynotears_MLP, self).__init__()
        assert len(dims) >= 2
        assert dims[-1] == 1
        d = dims[0]
        nums = dims[1]
    ...
    def forward(self, X, adj1, adj2):  # [n, d] -> [n, d]
        # M = X @ W + adj @ Xlags @ P
        M = torch.matmul(X[2:], self.w_est) + torch.matmul(X[1:-1], self.p1_est) + torch.matmul(X[:-2], self.p2_est)
...
def dual_ascent_step(model, X_torch, adj1, adj2, lambda1, lambda2, lambda3, rho, alpha, h, rho_max):
    ...
    while rho < rho_max:
        def closure():
            ...
            #print("loss: ", loss)
            #print ("primal_obj:", primal_obj)
            #print("model.h_func()", model.h_func())
            #print("h", h)
            primal_obj.backward()
            return primal_obj
...
def dynotears_model(model: nn.Module,
                      X,
                      adj1,
                      adj2,
                      lambda1: float = 0.,
    ...
    for _ in range(max_iter):
        rho, alpha, h = dual_ascent_step(model, X, adj1, adj2, lambda1, lambda2, lambda3,
                                         rho, alpha, h, rho_max)
...
def main():
    ...
    # 初始化model
    model = dynotears_MLP(dims=[d, n, 1], bias=True)
    model.to(device)
    ...
    W_est, P_est = dynotears_model(model, X, Xlags, adj, lambda1 = 0.1, lambda2 = 0.1)
